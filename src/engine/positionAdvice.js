...
function longTermPoints(intent, { triggers, health }) {
  ...
  if (intent === 'exit') {
    ...
    for (const t of thesisBreaks) points.push({ for: true, text: t.title })
    if (aboveRange) points.push({ for: true, text: aboveRange.title })
    if (upperThird) points.push({ for: true, text: upperThird.title })
    if (impliedVsGuidance) points.push({ for: true, text: impliedVsGuidance.title })
    if (fb?.available && fb.level >= 3) points.push({ for: false, text: fb.label })
    if (eb?.available && eb.direction === 'up') points.push({ for: false, text: eb.label })
    if (points.length === 0 && fb?.available) {
      points.push({ for: false, text: 'Nothing has fired against the thesis, and the price sits inside your own range' })
    }
  }
  ...
  if (intent === 'average-down') {
    ...
    for (const t of thesisBreaks) points.push({ for: false, text: t.title })
    if (fb?.available && fb.level >= 3) points.push({ for: true, text: fb.label })
    if (fb?.available && fb.level <= 1 && thesisBreaks.length === 0) points.push({ for: false, text: fb.label })
    ...
    if (eb?.available && eb.direction === 'up') points.push({ for: true, text: eb.label })
    if (eb?.available && eb.direction === 'down' && thesisBreaks.length === 0) {
      points.push({ for: false, text: `${eb.label} — the discount has narrowed, not widened` })
    }
  }
  ...
  if (intent === 'average-up') {
    ...
    for (const t of thesisBreaks) points.push({ for: false, text: t.title })
    if (aboveRange) points.push({ for: false, text: aboveRange.title })
    if (impliedVsGuidance) points.push({ for: false, text: impliedVsGuidance.title })
    if (fb?.available && fb.level >= 3) points.push({ for: true, text: fb.label })
    if (!aboveRange && !upperThird && fb?.available && fb.level >= 2) {
...
function shortTermPoints(intent, { health, technicals, suggestions }) {
  ...
  if (intent === 'exit') {
    if (tb?.available && tb.level <= 1) points.push({ for: true, text: tb.label })
    if (tb?.available && tb.level >= 3) points.push({ for: false, text: tb.label })
    if (rb?.available && rb.direction === 'down') points.push({ for: true, text: rb.label })
    if (rb?.available && rb.direction === 'up') points.push({ for: false, text: rb.label })
    if (resistanceTarget) {
      points.push({ for: true, text: `Near ${resistanceTarget.label.toLowerCase()} (${resistanceTarget.price})` })
    }
  }
  ...
  if (intent === 'average-down') {
    ...
    if (supportStop && !supportStop.tooClose) {
      points.push({ for: true, text: `Sitting near ${supportStop.label.toLowerCase()} (${supportStop.price})` })
    }
    ...
    if (tb?.available && tb.level <= 1 && !supportStop) {
      points.push({ for: false, text: 'No nearby support and the trend is still weak — a real risk of catching a falling knife' })
    }
  }

  if (intent === 'average-up') {
    if (tb?.available && tb.level >= 3) points.push({ for: true, text: tb.label })
    if (tb?.available && tb.level <= 1) points.push({ for: false, text: tb.label })
    if (rb?.available && rb.direction === 'up') points.push({ for: true, text: rb.label })
    if (rb?.available && rb.direction === 'down') points.push({ for: false, text: rb.label })
    if (sig.rsiOverbought) points.push({ for: false, text: 'RSI overbought — already extended' })
    if (resistanceTarget) {
      points.push({ for: false, text: `Approaching ${resistanceTarget.label.toLowerCase()} (${resistanceTarget.price})` })
    }
  }
